    ...
    def handle(self, *args, **options):
        ...
        for i in range(20):
            ...
            for _ in range(random.randint(0, 2)):
                UserChronicDisease.objects.create(
                    ...
                    severity=random.choice(["منخفض", "متوسط", "عالي"])
                )
        ...
        for u in users:
            Alert.objects.create(
                ...
                alert_type=random.choice(["معلومة", "طبي", "تحذير","حرج"])
            )

            PatientPriority.objects.get_or_create(
                patient=u,
                defaults={"score": random.randint(10, 90), "level": random.choice(["منخفض", "متوسط", "عالي", "حرج"])}
            )
